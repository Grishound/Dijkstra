...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
            if node not in d:
                Q.insert(NodeDistancePair(node, float('inf')))
                d[node] = float('inf')
            else:
                Q.insert(NodeDistancePair(node, 0))
        while len(Q) > 0:
            u = Q.extract_min()
            ...
            for v in u.node.adj:
                if v not in S:
                    if d[v] > d[u.node] + weight(u.node, v):
                        new = d[u.node] + weight(u.node, v)
                        Q.decrease_key(NodeDistancePair(v, d[v]), NodeDistancePair(v, new))
                        d[v] = new
                        parent[v] = u.node
        s_path = []
        node_count = 1
